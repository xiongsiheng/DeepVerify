...
def extract_final_graph(response):
    '''
    Extract the graph from the response.
    '''
    lines = response.split('\n')
    final_graph = {"Statement": {}, "Entailment": {}}
    for line in lines:
        pos_idf = len(line.split('":')[0]) + 2
        if 'graph' in line[:pos_idf].lower():
            graph = json.loads(line[pos_idf:])
            final_graph['Statement'].update(graph['Statement'])
            final_graph['Entailment'].update(graph['Entailment'])
...
def check_graph_structure(graph, print_error_type=False):
    ...
    for key in graph['Statement']:
        ...
        if graph['Entailment'][key] not in ['Given condition', 'Fact', 'Assumption', 'Evidence'] and not isinstance(graph['Entailment'][key], list):
            if print_error_type:
...
def my_gpt_completion(model_name, messages, timeout, max_tokens=128, wait_time=0, temperature=0.7, api_key=None, local_model_ls=None,
                      source=None):
    if source == 'local':
        raise Exception('Local model is not supported.')
    ...
    if model_name.startswith('gpt-5') or model_name.startswith('o3'):
    ...
    # rprint('[bold green]my_gpt_completion[/bold green]')
    # rprint('[bold red]Prompt:[/bold red]')
    # rprint(messages)
    # rprint('[bold blue]Response:[/bold blue]')
    # rprint(response)
    # rprint('[bold green]>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>[/bold green]')
